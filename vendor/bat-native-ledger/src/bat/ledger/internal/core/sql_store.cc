...
SQLStore::CommandFuture SQLStore::ExecuteMigration(int version,
...
SQLStore::CommandFuture SQLStore::Execute(const std::string& sql) {
  auto command = mojom::DBCommand::New();
  command->type = mojom::DBCommand::Type::EXECUTE;
  command->command = sql;
  return RunTransaction(std::move(command));
...
SQLStore::CommandFuture SQLStore::RunTransactionImpl(
  ...
  return CommandFuture::Create([&](auto resolver) {
